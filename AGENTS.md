...
| Signing                                | The Wallet API (`@ledgerhq/wallet-api-client-react`) and nothing else. No private key exists in the Live App. A holder whose account is EIP-7702-delegated to `Simple7702Account` signs a multi-call session as one `executeBatch` to their own address — still through the Wallet API; `sessionCalls` in `packages/core` decides from the account's code, and the app never delegates. (`apps/probe-7702` talks to a device over WebHID with Ledger's DMK — it is the dev instrument where that delegation happens, not the Live App, and nothing in `apps/live-app` imports it) |
...
`pnpm ledger:screens -- --probe` renders the EIP-7702 batch on top
(`screens/batch7702*`, `screens/results-probe.json`) without touching the flow's
captures; its descriptor, `calldata-Simple7702Account.json`, is bound to the
delegate's address and so never matches the six flow transactions. The registry
copies of every descriptor live in `ethereum/clear-signing-erc7730-registry`
(PRs #2953 and #2954), in the registry's v2 schema — a change here is a change
