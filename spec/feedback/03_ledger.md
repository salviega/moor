...
**Reportado:** pendiente. Sugerencias: (1) documentar el formato `calldata` anidado en la guía de Clear Signing, con la versión mínima de la app y un ejemplo con `executeBatch`; (2) que el tester diga *qué* campo hace "parcial" un veredicto; (3) publicar un descriptor para `Simple7702Account.executeBatch` — Ledger lo acepta como único delegado y "smart account upgrade" es una función suya, pero cada batch que ese delegado ejecuta se firma a ciegas; (4) documentar si el metadata service resuelve descriptores para EOAs delegadas por 7702 (`ProxyContextFieldLoader`), que es lo que haría útil el punto 3; (5) documentar que `signTransaction` acepta tipo 4.
