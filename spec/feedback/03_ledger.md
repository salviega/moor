...
**Impacto en Moor:** abre el camino de "un flujo, una firma" del [08 §2.3](../definicion/08_roadmap.md#23-smart-accounts-y-eip-7702) — cierra el riesgo de que el batch fuese ilegible, que era el que lo descartaba. Queda una sola incógnita, y es de Ledger (la resolución por `to` delegada). `apps/probe-7702` existe para preguntársela a la Flex física.

**Reportado:** pendiente. Sugerencias: (1) documentar el formato `calldata` anidado en la guía de Clear Signing, con la versión mínima de la app y un ejemplo con `executeBatch`; (2) que el tester diga *qué* campo hace "parcial" un veredicto; (3) documentar si el metadata service resuelve descriptores para EOAs delegadas por 7702 — es la pieza que decide si una smart account de Ledger puede clear-signar un batch.
