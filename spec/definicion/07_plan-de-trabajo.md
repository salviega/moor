...
- ~~**Key Ring enrolado** en el host donde va a correr el agente.~~ **Cerrado por otro camino el 6 sep:** el agente corre en Supabase con los secretos del proyecto (fase 4); Key Ring en un host propio, al [08](./08_roadmap.md). El portátil sigue siendo miembro.
...
**Fase 0 cerrada el 5 de septiembre**, un día antes del calendario, con dos ítems que se mueven de fase a propósito (`quote()` → fase 1, descriptores en el dispositivo → fase 3) y uno que esperaba al host —el Key Ring en el VPS— y se cerró el 6 sep por otro camino (Supabase, fase 4).
...
- ~~Las cinco pantallas del [04 §5](./04_diseno-de-solucion.md#5-pantallas), en inglés: **Positions**, **New position**, **Review & sign**, **Position detail**, **First-time setup**.~~ *Rediseño del 6 sep (dirección "cuaderno de bitácora"): un acento ámbar solo para "requiere tu firma", gráfico de rango con 48 h de Chainlink, banda de propuesta pendiente, revisión de firma sin jerga, todos los estados enumerados; verificado a 1440 y 390 px. Segunda pasada el mismo día: dashboard de dos columnas (lista a la izquierda, posición seleccionada en paneles a la derecha) y control de cuenta (*Switch* / *Disconnect*).* Hechas el 5 sep (`apps/live-app/src/app/`): Positions lista los nombres del registry del holder (`LabelRegistered`) con estado, % convertido y pulso del agente; New position → Review & sign explica qué pasa y qué no y lista cada firma con lo que la Ledger muestra; Position detail con panel del agente, *Close* (dock + unregister) y *Revoke agent*; First-time setup como pantalla de estado (recorte permitido: las firmas de la primera vez siguen en `SetupHolder.s.sol`).
...
**Objetivo:** un proceso headless sin USB — una Edge Function en Supabase que `pg_cron` despierta cada cinco minutos — que vigila, escribe en su subnombre, propone una vez, y que verificablemente no puede hacer más.
...
- ~~**Umbrales v1**, fijos y documentados en el código; al cruzar uno, **una** llamada a `claude-opus-5` con `messages.parse()` contra el esquema zod de la propuesta, thinking adaptativo, `fallbacks: "default"`. Si la salida no valida, no se escribe.~~ `packages/core/src/agent.ts` (`THRESHOLDS`, `detectTriggers`, `deterministicProposal`, `simulate`, `proposalPrompt`, `shouldPropose`), 25 pruebas; `apps/agent/src/model.ts` llama a `client.beta.messages.parse` con `betaZodOutputFormat(Proposal)`, `fallbacks: "default"` y thinking adaptativo; una refusal o una salida que no valida caen a la propuesta determinista. La llamada real a Claude está por probar (falta la API key en el entorno). *6 sep:* la llamada pasa a **Groq** (`openai/gpt-oss-120b`, JSON Schema estricto, tier gratuito) para no gastar durante el hackathon — `apps/agent/src/model.ts` es un `fetch`; el esquema estricto y el parseo viven en `packages/core/src/model.ts` con sus pruebas. Sigue pendiente la primera llamada real (falta `moor/groq-api-key` en el ring).
- ~~Secretos por **Key Ring** (`wallet-cli ring`) en el VPS: llave del agente, RPC, `GROQ_API_KEY`. `systemd` para el bucle.~~ *6 sep:* **el agente va a Supabase.** El ciclo se extrajo a `apps/agent/src/cycle.ts` (compartido); `edge.ts` lo corre una vez por petición como Edge Function `agent-cycle` (Deno), `pg_cron` + `pg_net` la llaman cada 5 min (`supabase/migrations/…_agent_cron.sql`, URL y anon key desde Vault), y los secretos son del proyecto (`pnpm agent:secrets`). Probado en seco: el bundle (`pnpm agent:bundle`, esbuild) corre bajo Deno y bajo el edge-runtime de Supabase en local — dos posiciones leídas, ~5 s por ciclo. Key Ring en un host propio pasa al [08](./08_roadmap.md): el enrolamiento sin USB no está documentado y no había por qué esperar. Los assets del VPS (`deploy/run.sh`, el unit de systemd) se retiraron.
...
2. El modelo en el agente → la propuesta queda como texto determinista.
