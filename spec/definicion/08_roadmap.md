...
- *Lo que desbloquea el camino es el registro, y podemos empujarlo nosotros:* dos PRs a `clear-signing-erc7730-registry` — los descriptores de Moor (pendientes desde el 03) y el de `Simple7702Account.executeBatch` con `calldata` anidado (`descriptors/probe/`), que serviría a cualquier dApp que use el delegado de Ledger. Publicados, la sesión de una firma se ve en palabras en cualquier Flex; hasta entonces, la evidencia vive en Speculos.
