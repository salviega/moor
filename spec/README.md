...
- **Ledger:** humano en el loop como núcleo, Clear Signing con ERC-7730 para todo lo que se firma. x402 no. Key Ring para los secretos del agente era el plan; el 7 de septiembre pasaron a Supabase y Key Ring en un host propio queda en el [08](./definicion/08_roadmap.md).
...
**Documentación completa del 01 al 08. Construcción: fases 0 a 4 cerradas** ([07](./definicion/07_plan-de-trabajo.md)) — 0 y 1 el 5 de septiembre, 2 (ENSv2) también el 5, 3 (Live App) el 6 — `btc-dip-2.salviega.eth` se creó desde Ledger Live con la Flex y las seis firmas están capturadas en Speculos con nuestros descriptores (cinco clear-signed) — y **4 (el agente) el 7, siete días antes del calendario**: corre como Edge Function en Supabase, `pg_cron` la despierta cada cinco minutos sin dispositivo ni proceso local, y el primer tick disparó dos escrituras reales en Sepolia. Monorepo y CI en verde, Aqua/SwapVM redesplegados en Sepolia (Aqua con `exact_match` en Sourcify), `salviega.eth` en ENSv2, Live App en [Vercel](https://getmoor.vercel.app), Speculos verificado. Sigue la fase 5 (demo, submission). Cierre de submissions el 13 de septiembre de 2026. Los pendientes de cada tema están al final de su documento; los que atraviesan varios (fuente de precio, enumerar subnombres, confirmar el redespliegue con 1inch) tienen fase asignada en el 07.
